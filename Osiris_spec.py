...
class Skew_spec:
    ...
    def __init__(self,S_cube,S_sqd_cube,eta,u_arr,v_arr,nu_o,dnu,dnu_f,weights_cube=None,nu_21=nu_21,cosmo=None):
        ...
        #self.data = S_cube
        #self.Skew_cube = (S_sqd_cube**2)*np.conjugate(S_cube) # [Jy^2 Hz^2]
        self.Skew_cube = S_sqd_cube*np.conjugate(S_cube) # [Jy^2 Hz^2]
        #self.Skew_cube = S_sqd_cube*S_cube # [Jy^2 Hz^2]
        #self.Skew_cube = (S_cube**2)*np.conjugate(S_cube) # [Jy^2 Hz^2]
    ...
    @staticmethod
    def Skew2Tb(dnu,dnu_f,nu_o,z,cosmo,verbose=True):
        """
        ...
        #conv_factor = (N_chans**3) * (lam_o**6/(8*kb**3)) * (1/(fov*dnu)**2) * co_vol # [K^3 Mpc^3 h^-3]
        #conv_factor =  dnu * (lam_o**6/(8*kb**3)) * (1/(fov)**2) * co_vol * 1e+9 # [mK^3 Mpc^3 h^-3]
        conv_factor =  dnu * (lam_o**6/(8*kb**3)) * (1/(fov)) * co_vol * 1e+9 # [mK^3 Mpc^3 h^-3]
        #conv_factor =  dnu * (lam_o**6/(8*kb**3)) * (1/(fov)**4) * co_vol * 1e+9 # [mK^3 Mpc^3 h^-3]
    ...
    def Spherical(self,wedge_cond=False,N_bins=60,log_bin_cond=False,kr_min=None,kr_max=None,
        ...
        Cosmo_factor = Skew_spec.Skew2Tb(dnu,dnu_f,self.nu_o,self.z,self.cosmo,verbose=True)
        ...
        #Skew_spec1D = Skew_spec1D/(dnu_f**3) # Might not need this step.
        Skew_spec1D = Skew_spec1D/(dnu_f**2) # Might not need this step.
        
        print('Mean skew spec = %5.3e [Jy^3]' % np.nanmean(Skew_spec1D))
        print('Max skew spec = %5.3e [Jy^3]' % np.nanmax(Skew_spec1D))
    ...
    def Cylindrical(self):
        ...
        dnu = self.dnu*1e+6 #[Hz] full bandwidth.
        dnu_f = self.dnu_f*1e+6 #[Hz] fine channel width.
        Cosmo_factor = Skew_spec.Skew2Tb(dnu,dnu_f,self.nu_o,self.z,self.cosmo)

        Skew_spec2D = Skew_spec2D/(dnu_f**3)
        # Assigning the power.
    ...
    def plot_spherical(k_r,Skew_spec1D,figsize=(8,6),xlim=None,ylim=None,title=None,figaxs=None,\
        ...
        if ylabel:
            ...
            axs.set_ylabel(r'$\rm{S(k)} \, [\rm{mK^3}\,\it{h^{-3}}\,\rm{Mpc^3}]$',fontsize=24)
    ...
    def plot_cylindrical(Skew_spec2D,kperp,kpar,figsize=(7.5,10.5),cmap='viridis',
        ...
        if clab:
            ...
            cb.set_label(label=r'$\rm{S(k_\perp,k_{||})} \, [\rm{mK^3}\,\it{h^{-3}}\,\rm{Mpc^3}]$',fontsize=20)
